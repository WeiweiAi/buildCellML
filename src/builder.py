...
def add_component(model, component):
    ...
    if model.containsComponent(component):
        ...
        return model.addComponent(component)
...
def add_variable(component,variable):
    ...
    if component.hasVariable(variable):
        print('The variable {} already exists in the component {}!'.format(variable.name(),component.name()))
        return False
    else: 
        return component.addVariable(variable)
...
def varmap(var1,var2,connection=True):
    ...
    if connection:
        return Variable.addEquivalence(var1, var2)

    else:
        return Variable.removeEquivalence(var1, var2)

def check_equivalence(var1,var2):
    """ 
    Check if two variables are equivalent.
    Args:
        var1 (:obj:`Variable`): the first variable to be checked
        var2 (:obj:`Variable`): the second variable to be checked
    Returns:
        :obj:`bool`: whether the variables are equivalent
    """
    return var1.hasEquivalentVariable(var2)

def check_equivalence_list(vars):
    """ 
    Check if a list of variables are equivalent.
    Args:
        vars (:obj:`list`): the list of variables to be checked
    Returns:
        :obj:`bool`: whether the variables are equivalent
    """
    for i in range(len(vars)):
        for j in range(i+1,len(vars)):
            if not vars[i].hasEquivalentVariable(vars[j]):
                return False
    return True
...
def importComponents(model,importSource,import_components_dict):
    ...
    for component in list(import_components_dict.keys()):
        ...
        c.setSourceComponent(importSource,import_components_dict[component])
        if c.isResolved() and  model.addComponent(c):
            pass
        else:
